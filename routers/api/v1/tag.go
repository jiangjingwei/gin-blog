...
func AddTag(c *gin.Context) {
	...
	valid.Required(createBy, "createb_by").Message("创建人不能为空")
	valid.MaxSize(name, 100, "createb_by").Message("创建人最长为100个字符")
	...
	if !valid.HasErrors() {
		...
		for _, err := range valid.Errors {
			log.Printf("data valid err, key: %v, message: %v", err.Key, err.Message)
		}
